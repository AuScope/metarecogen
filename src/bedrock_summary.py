#!/usr/bin/env python3
import json
import sys

import boto3
import botocore

"""
Use Claude V2.0 model to summarize text via AWS Bedrock and 'boto3' package
"""

MODELS = { 
    # Claude model
    "claude" : {
        "maxTokens": 200000,
        "modelId": "anthropic.claude-v2",
        "params": {
            "max_tokens_to_sample": 300,
            "temperature": 0.1,
            "top_p": 0.9
        },
        "prompt": """

Human: You are writing a report about geology and geological models. Please provide a two paragraph summary of the following text. Do not add any information that is not mentioned in the text below.

<text>
{text}
</text>

Assistant:
    """
    },
}


def run_claude(text: str) -> str:
    """ Run claude summarizer

    :param text: text string to be summarized
    :returns: summary text
    """
    try:
        brt = boto3.client(service_name='bedrock-runtime')
        body, modelId = run_model('claude', brt, text)
        accept = 'application/json'
        contentType = 'application/json'
        response = brt.invoke_model(body=body, modelId=modelId, accept=accept, contentType=contentType)
    except botocore.exceptions.BotoCoreError as bce:
        print("AWS boto3 error:", bce)
        sys.exit(1)
    try:
        response_body = json.loads(response.get('body').read())
    except json.JSONDecodeError as jde:
        print("Could not decode response from AWS:", jde)
        sys.exit(1)
    summary = response_body.get('completion', '')
    return "Generated by Anthropic Claude V2.0: " + summary


def run_model(model_name: str, brt: botocore.client.BaseClient, text: str) -> (str, str):
    """ Runs a model using config

    :param model_name: name of model
    :param brt: AWS boto3 client object, connected to 'bedrock-runtime'
    :param text: text string to be summarized
    """
    config = MODELS[model_name]
    body = json.dumps({
        "prompt": config["prompt"].format(text=text),
        **config["params"]
    })
    return body, config['modelId']
